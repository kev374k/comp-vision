...
def rnn_step_backward(dnext_h, cache):
    ...
    # (N, H) @ (H, H)
    dprev_h = (backward_dv) @ np.transpose(Wh)

    # (D, N) @ (N, H)
    dWx = np.transpose(x) @ (backward_dv)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        input_vector = x[:, t, :]
        prev_h, sub_cache = rnn_step_forward(input_vector, prev_h, Wx, Wh, b)
        h[:, t, :] = prev_h
...
def rnn_backward(dh, cache):
    ...
    for t in range(T - 1, -1, -1):
        ...
        # dh0 represents the previous gradients
        dh0 += dh[:, t, :]
        curr_dx, dh0, curr_dWx, curr_dWh, curr_db = rnn_step_backward(dh0, curr_cache)
        dx[:, t, :] = curr_dx
        dWx += curr_dWx
        dWh += curr_dWh
        db += curr_db
